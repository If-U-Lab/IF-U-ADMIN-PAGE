...
const KPICard: React.FC<{ title: string; value: string; trend: string; isUp: boolean }> = ({ title, value, trend, isUp }) => (
  <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
    ...
    <div className="flex items-baseline gap-2 mt-2">
      ...
      <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${isUp ? 'text-blue-600 bg-blue-50' : 'text-red-600 bg-red-50'}`}>
        {isUp ? '▲' : '▼'} {trend}%
      </span>
    </div>
    <p className="text-xs text-gray-400 mt-2">전주 대비</p>
  </div>
);
...
const RankingCard: React.FC<{ rank: number; title: string; category: string; score: number }> = ({ rank, title, category, score }) => (
  <div className="flex items-center gap-4 p-4 hover:bg-gray-50 transition-colors rounded-xl">
    <div className={`w-8 h-8 flex items-center justify-center rounded-lg font-bold text-lg ${rank === 1 ? 'bg-black text-white' : 'text-gray-400'}`}>
      {rank}
    </div>
    ...
    <div className="text-right">
      ...
      <p className="text-[10px] text-gray-400 uppercase tracking-wider">Dopamine Score</p>
    </div>
  </div>
);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-bold text-lg">사용자 추이</h3>
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-bold text-lg">활성 사용자 분석</h3>
            </div>
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="space-y-8">
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
            <h3 className="font-bold text-lg mb-6">🔥 도파민 랭킹</h3>
            <div className="space-y-4">
              ...
              <button className="w-full mt-4 py-3 text-sm font-semibold text-gray-500 hover:text-black transition-colors">
                순위 더보기 →
              </button>
            </div>
          </div>
          ...
          <div className="bg-black text-white p-6 rounded-2xl shadow-xl">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="font-bold text-lg">인기 카테고리</h3>
                <p className="text-gray-400 text-sm mt-1">현재 가장 뜨거운 주제</p>
              </div>
              <span className="bg-red-500 text-[10px] font-bold px-2 py-1 rounded">HOT</span>
            </div>
            <div className="mt-8">
              <span className="text-4xl font-bold">시사/사회</span>
            </div>
